...
namespace SharpNeatLib.NeatGenome
{
	...
	{	
		...
		public override IGenome CreateOffspring_Sexual(EvolutionAlgorithm ea, IGenome parent)
		{
            ...
            // Schrum: No longer true if we are deleting output modules from module mutation.
            ...
            // Schrum: need to watch out for disjoint output neurons if modules are being deleted
            if (ea.NeatParameters.pModuleDeletion > 0)
            {
                idxBound = otherParent.neuronGeneList.Count;
                for (int i = 0; i < idxBound; i++)
                {
                    if (otherParent.neuronGeneList[i].NeuronType != NeuronType.Hidden)
                    {
                        // Don't include the neurons that are already present
                        if (!newNeuronGeneTable.Contains(otherParent.neuronGeneList[i].InnovationId)) {
                            newNeuronGeneList.Add(new NeuronGene(otherParent.neuronGeneList[i]));
        ...
        private void Module_Deletion(EvolutionAlgorithm ea)
        {
            ...
            if (numModules > 1) // Only delete if there are more than 1
            {
                ...
                for (int i = 0; i < outputsPerPolicy; i++)
                {
                    ...
                    Debug.Assert(neuronGeneList[neuronIndex + i].NeuronType == NeuronType.Output, "Only delete output neurons with module deletion.");
                    idsToDelete[i] = id;
                }
            ...
            Debug.Assert(this.outputNeuronCount > 0 && numModules > 0, "No outputs or modules left after module deletion!");
        }

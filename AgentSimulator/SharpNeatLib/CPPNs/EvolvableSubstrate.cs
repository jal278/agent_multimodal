...
namespace SharpNeatLib.CPPNs
{
     ...
     * 
     * The Evolvable-Substrate HyperNEAT implementation. 
     **/
    ...
    {     
        ...
        class Rect : IComparable<Rect>
        {
            ...
            public void createTree(int resolution)
            {
                ...
                while (true)    //TODO replace. Do not calculate distance for every rectangle
                {
                  //  (squares<(resolution*resolution))//
                    rectToAdd.Clear();
                    if (rectList.Count < (resolution*resolution)) //(Math.Abs(rectList[0].x1 - rectList[0].x2) > 0.4f))// || (sampleTreshold != -1.0f && dif > sampleTreshold))
                    {
        ...
        public void generateConnections(List<PointF> inputNeuronPositions, List<PointF> outputNeuronPositions,
            INetwork genome, int initialResolution, float varianceThreshold, float bandThreshold, int ESIterations,
                                                float divsionThreshold, int maximumResolution,
                                                ...
                                                ref  ConnectionGeneList connections, ref List<PointF> hiddenNeurons)
        {
            ...
            float connectionThreshold = 0.0f;
            ...
            foreach (PointF input in inputNeuronPositions)
            {
                ...
                startRec.createTree( (int) initialResolution);
            ...
            foreach (ExpressPoint t in _connections)
            {
                ...
                weight = (float)( output * HyperNEATParameters.weightRange); // (float)(((Math.Abs(output) - (0.0f)) / (1 - 0.0f)) * HyperNEATParameters.weightRange * Math.Sign(output));
            ...
            //Iterativly discover new hidden nodes from the already discovered ones
            foreach (ExpressPoint t in _connections)
            {
